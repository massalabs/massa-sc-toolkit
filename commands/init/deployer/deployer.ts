...
export class Deployer {
    ...
    async init() {
        ...
        const privateRpc =
            process.env.JSON_RPC_URL_PRIVATE || DEFAULT_PRIVATE_RPC;
    ...
    async deployContract(contractWasm: string) {
        ...
        const operationId = await this.web3Client
            .smartContracts()
            .deploySmartContract(
                {
                    coins: 0,
        ...
        console.log(
            `Operation submitted successfully to the network. Operation id: ${operationId[0]}\n`
        );
        console.log(
        ...
        const event = await this.web3Client
            .smartContracts()
