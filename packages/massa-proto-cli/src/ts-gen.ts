...
export function generateTSCaller(
  ...
  const responseType = protoFile.resType !== null ? returnType[protoFile.resType] : 'void';
 ...
 *
 * @returns {${returnType[protoFile.resType]? returnType[protoFile.resType] : 'void'}} The result of the "${protoFile.funcName}" function.
 */
 export async function ${protoFile.funcName}(${args}): Promise<${responseType}> {
  ${checkUnsignedArgs}
