...
export function generateTSCaller(outputPath: string, helperRelativePath: string, protoFile: IProtoFile, contractAddress?: string){
    ...
    for (let arg of protoFile.argFields){
        if(arg.type === "uint32" || arg.type === "uint64" || arg.type === "fixed32" || arg.type === "fixed64"){
