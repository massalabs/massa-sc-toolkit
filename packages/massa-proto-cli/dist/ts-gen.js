 ...
 */
function setupArguments(protoFile, mode) {
    if (mode != 'web3' && mode != 'wallet')
        throw new Error('Error: mode must be either "web3" or "wallet".');
    const account = mode == 'web3' ? 'account: SmartContractsClient' : 'account: IAccount';
 ...
 */
function generateTSCaller(outputPath, protoFile, providerUrl, contractAddress, mode) {
    // check the mode
    if (mode != 'web3' && mode != 'wallet')
        throw new Error('Error: mode must be either "web3" or "wallet".');
    ...
    const args = setupArguments(protoFile, mode);
...
function generateTsCallers(protoFiles, outputDirectory, providerUrl, address, mode) {
    for (const file of protoFiles) {
        ...
        generateTSCaller(outputDirectory, file, providerUrl, address, mode);
    }
}
