...
function generateCommonHelperFile(outputPath, mode) {
 ...
 * @param contractName
 */
function generatePollerFunction(protoFile, mode, outputPath, contractName) {
...
`;
        ...
        (0, fs_1.writeFileSync)(`${outputPath}${protoFile.funcName}Helper.ts`, helperImports
            + "\n/*****************GENERATED BY PROTOC*****************/\n\n"
            + helperFile, 'utf8');
        console.log(`Helper file: ${protoFile.funcName}Helper.ts updated at: ${outputPath}`);
    }
}
function generateCommonCallerFile(protoFiles, outputPath, mode, contractName) {
 ...
 */
function generateTsCallers(protoFiles, outputDirectory, address, mode, contractName) {
    // check the mode
...
`;
    ...
    (0, fs_1.writeFileSync)(`${outputDirectory}${fileName}`, part1 + part2 + part3, 'utf8');
}
