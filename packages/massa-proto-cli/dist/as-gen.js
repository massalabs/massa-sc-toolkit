...
function generateAsCall(protoData, address, outputDirectory) {
    ...
    // generating caller
    let caller = `export function ${protoData.funcName}(${args.length > 0 ? args.join(', ') + ', coins: number' : 'coins: number'}) : ${protoData.resType !== null ? protoData.resType : 'void'} {\n`;
    caller += `  const result = call("${address}", "${protoData.funcName}", changetype<StaticArray<u8>>(encode${protoData.funcName}Helper(new ${protoData.funcName}Helper(${Array.from(protoData.argFields.keys()).join(', ')}))), coins);\n`;
    if (protoData.resType !== null) {
    ...
    caller += '}';
    writeFileSync(generateAsImports(protoData) + caller, path.join(outputDirectory, protoData.funcName + '.ts'));
}
function generateAsHelper(protoData, outputDirectory) {
    ...
    if (protocProcess.status !== 0) {
        console.error(`Failed to generate AS helpers code for ${protoData} with error: ${protocProcess.stderr}`);
    }
}
export function generateAsCallers(protoFiles, address, outputDirectory) {
