...
import { ClientFactory, ProviderType, WalletClient } from '@massalabs/massa-web3';
...
function displayHelp() {
    ...
    console.log('\t\t--gen sc|web3\t- the generation mode for contracts callers (sc) or web3 app (web3)');
    console.log('\t\t--addr contract_public_key\t- the public address of the contract to interact with');
    console.log('\t\t[--out output_directory]\t- optional output directory for the callers to generate (default: .)');
    process.exit(1);
}
function getGenMode(args) {
...
async function run(args) {
    ...
    let pFiles = await client.smartContracts().getProtoFiles([address], out);
